...
MalType * EVAL(MalType * AST, env_t * repl_env);
char* PRINT(MalType * AST);
char* rep(char * line, env_t * repl_env);

MalType * EVAL_symbol(MalType * AST, env_t * repl_env);
...
int main(void) {
    ...
    repl_env->map = GC_MALLOC(sizeof(map_t));
        ...
        case MAL_SYMBOL: {
            ...
            case MAL_LIST: {
                ...
                while (element != NULL) {
                    ...
                    // }
                    append(list, (void *)evaluated_element->value.IntValue, sizeof(*(evaluated_element->value.IntValue)));
                    element = element->next;
                };
...
MalType * EVAL_symbol(MalType * AST, env_t * repl_env) {
    // NOTE: return the function from the repl env
    MalSymbol * operator = AST->value.SymbolValue;
    MalType * operation = get(repl_env, operator);
    return operation;
}
