            ...
            case MAL_FN_WRAPER: {
                ...
                while (binds != NULL && exprs != NULL) {
                    ...
                    if (exprs->next == NULL && binds->next != NULL) {
                        printf("expr is NULL\n");
                        char *bind =
                            ((MalType *)binds->next->data)->value.SymbolValue;
                        if (strcmp(bind, "&") != 0) {
            ...
            case MAL_SYMBOL: {
                ...
                if (strcmp(symbol, "let*") == 0) {
                    ...
                    if (((MalType *)(element->next->data))->type == MAL_LIST ||
                        ((MalType *)(element->next->data))->type ==
                            MAL_VECTOR) {
                        bindings_list =
        ...
        case MAL_VECTOR: {
            ...
            case MAL_FN_WRAPER: {
                ...
                while (binds != NULL && exprs != NULL) {
                    ...
                    if (strcmp(bind, "&") == 0) {
                        ...
                        // make the next exprs a vector (wraped in a maltype)
                        // that contain the rest of the exprs
                        // define it
                        MalType *wrap         = GC_MALLOC(sizeof(MalType));
                    ...
                    if (exprs->next == NULL && binds->next != NULL) {
                        ...
                        // if there is no exprs, still wrap a empty vector
                        // (node_t *) in a MalType->type == MAL_VECTOR
                        MalType *wrap         = GC_MALLOC(sizeof(MalType));
                        wrap->type            = MAL_VECTOR;
                        wrap->value.ListValue = GC_MALLOC(sizeof(node_t));
            ...
            case MAL_SYMBOL: {
                ...
                if (strcmp(symbol, "fn*") == 0) {
                    ...
                    // create the function
                    MalFn *function = GC_MALLOC(sizeof(MalFn));
                    // MalList * parameters = ((MalType
                    // *)element->next->data)->value.ListValue;
                    MalType *parameters = element->next->data;
                    MalType *body       = element->next->next->data;
